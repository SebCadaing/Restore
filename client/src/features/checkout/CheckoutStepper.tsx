...
const steps = ["Addrress", "Payment", "Review"];
...
export default function CheckoutStepper() {
  ...
  const [confirmationToken, setConfimationToken] = useState<ConfirmationToken | null>(null);
  ...
  const handleNext = async () => {
    ...
    if (activeStep === 1) {
      ...
      setConfimationToken(stripeResult.confirmationToken);
    }
    if (activeStep === 2) {
  ...
  const createOrderModel = async () => {
    ...
    if (!shippingAddress || !paymentSummary) throw new Error("Probelm creating order");
